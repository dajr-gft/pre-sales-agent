...
class TestWorker:
    ...
    def test_instruction_provider_includes_skill_md_and_protocol(self):
        ...
        assert 'activity_phases' in instr
        assert 'deliverables' in instr
        assert 'timeline' in instr

    def test_instruction_provider_injects_manifest_and_prior_requirements(self):
        """Step B depends on the FR/NFR generated by Step A — both must
        appear as labelled blocks so the worker can derive activities
        and deliverables that actually anchor the requirements."""
    ...
    def test_instruction_provider_aborts_when_prior_requirements_missing(self):
        """Manifest alone is not enough — delivery plan needs the FRs
        before it can produce anchored activities. Without them the
        provider must trigger the MISSING_INPUT path."""

        class _Ctx:
            state = {'extraction_manifest': {'project_name': 'Test'}}
