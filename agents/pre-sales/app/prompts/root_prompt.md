...
- `status == "exhausted"` → The loop spent its round budget without converging. The envelope's `unresolved_items` lists the top blocking problems it could not stabilize — each item carries `affected_sections`, `affected_fields`, and a `short_reason` (no `decision_type`: this is non-convergence, not a pending decision). Use ONLY these items to understand what stuck; translate each into plain, consultant-style language (never the field names, severities, or finding categories verbatim) and let the user decide whether to accept the SOW as-is, restart, or hand off to a human reviewer. You cannot read findings from state, so do NOT try; if `unresolved_items` is absent, fall back to the envelope's `summary` (also never relayed verbatim). Do NOT call `sow_quality_loop` again with the same staged payload — re-staging is required first.
