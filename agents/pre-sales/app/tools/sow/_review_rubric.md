<title>SOW Semantic Review — Rubric</title>
...
These rules apply to EVERY review, without exception. They are not stylistic preferences — they are the operating contract for this pass. Violating any rule below invalidates the review.

- **ALWAYS read the SOW end to end before emitting any finding.** Walk EVERY Functional Requirement, EVERY Non-Functional Requirement, EVERY Out-of-Scope item, EVERY Assumption, EVERY Deliverable, EVERY Activity Phase and its tasks, EVERY Architecture component, EVERY Architecture integration, EVERY Success Criterion, EVERY Risk, EVERY Partner Role, EVERY Customer Role, EVERY Milestone, the Executive Summary, the Partner Overview, the Customer Overview, the Architecture Description, and the Timeline. Skipping any of these is a protocol violation. Partial reading is the dominant cause of both false positives (missing a disambiguation clause) and false negatives (missing a real contradiction in a section you did not scan).
- **ALWAYS read each item literally to its end.** NEVER quote only the leading clause. If an Out-of-Scope item, an Assumption, an FR, or any other item contains a trailing "except for [X]" / "exceto pelo [X]" / "salvo o [X]" / "unless" / "other than" / "with the exception of" clause, you ONLY discover it by reading the item to its last word. Truncated reading is the dominant root cause of BLOCKER false positives in production.
- **ALWAYS walk every pair in the contradictions matrix (section 2) against the SOW.** Do not stop after finding one contradiction; check ALL six pairs. Finding zero contradictions in a pair is a valid outcome — skipping the check is not.
- **ALWAYS verify every pattern in section 3 (3a vulnerabilities, 3b required limitations, 3c semantic gaps) against the entire SOW.** Each pattern must be considered. Finding zero violations of a pattern is a valid outcome — skipping the pattern is not.
- **ALWAYS apply the final calibration check (section D) before emitting any finding.** No exceptions.
- **NEVER assume a section is correct because another section looks correct.** Each section is independently subject to the rubric. Naming consistency, contradictions, and required disclosures must be verified per-section.
- **NEVER short-circuit because the SOW is long.** If the input is large, allocate more reasoning to coverage. The hard cap of 12 findings in the output applies to EMISSION, not to READING — you read 100% of the input regardless of how many findings you end up emitting.
- **NEVER paraphrase the SOW when quoting evidence.** Quote literal text. Paraphrase hides exception clauses and breaks the reader's ability to verify the finding without opening the SOW.
</mandatory_completeness_protocol>
...
- **Schedule-graph inconsistency.** A deliverable is scheduled in a phase that conflicts with the phase where its preconditions are met. Examples of preconditions: the activity that produces the deliverable's content; the integration or system the deliverable documents; the customer-side resource the deliverable depends on. If a deliverable describes a system that is configured in Phase 1 but is itself scheduled to land in Phase 3, the customer team has weeks of operating without documentation — flag the misalignment with both phase IDs in the evidence.
- **Reference to a parent contract without complete identification.** When the SOW mentions a master agreement, framework agreement, or similar parent contract under which it operates (explicitly or implicitly), the reference must be complete: name, identifier, and a precedence statement (which document governs in case of conflict). A bare phrase like "governed by the existing agreement" without naming what is being referenced is a contractual exposure — neither party has a stable anchor in case of dispute.
</contractual_exposures>

<required_limitation_disclosures>
...
- Style-guide rule compliance — minimum item counts, ID formats, required wording, role description length, assumption consequence-clause presence, deliverable `format` declaration and enum value — handled by `ContentValidator` mechanical checks. The agent already sees these as `errors` and `warnings` in the same payload that carries your findings.
...
2. Both cited anchors are quoted with enough literal text for a human reader to verify the conflict without opening the SOW. Quote the FULL text of the anchor — never just the leading clause. Truncating before a trailing "except for [X]" / "exceto" / "salvo" / "unless" clause is a finding-generation error, not a stylistic shortcut.
...
4. **Neither cited anchor contains a disambiguation clause for the other.** An OOS item ending with "except for [in-scope item ID or name]" (or equivalent: "exceto pelo X", "salvo o X", "unless", "other than", "with the exception of") explicitly removes the contradiction; the disambiguation IS the contract. If you see one of these clauses naming the in-scope side of the alleged conflict, the finding cannot be `BLOCKER` — and most likely is not a finding at all. Re-read the OOS item to its end before classifying.

If any of these conditions fails, the maximum severity is `MAJOR`. If the evidence does not clearly support `MAJOR` either, the finding is `MINOR`. If condition 4 fails because the disambiguation is clear and complete, the finding is dropped entirely.
...
<patterns_that_look_like_findings_but_are_not>
...
<required_template_wording_and_disclosures>
...
- **Disambiguation between OOS exclusions and any in-scope item** — when an OOS exclusion describes territory adjacent to an in-scope item (Functional Requirement, NFR, Deliverable, Activity, Architecture component or integration), look for disambiguation BEFORE flagging contradiction. Two patterns are intentional contract, NOT findings:

   - **Explicit exception clauses inside the OOS item.** An OOS item that contains "except for [in-scope item]", "exceto pelo [in-scope item]", "salvo o [in-scope item]", "unless", "other than", "with the exception of", or any equivalent in any language, followed by an in-scope item reference, EXPLICITLY removes the conflict. The disambiguation IS the contract. **You MUST read every OOS item to its end before quoting it as evidence — quoting only the leading exclusion clause and skipping a trailing "except for [X]" carve-out is the most common BLOCKER false positive.** If your evidence excerpt is shorter than the literal OOS item, re-read the item and verify there is no exception clause naming the in-scope side of the alleged contradiction.
   - **Scope-distinction patterns.** When the in-scope item names one capability and the OOS excludes a related-but-distinct capability (e.g., "RAG over Confluence is in scope (FR-06); RAG over other repositories is out of scope"), the disambiguation IS the contract.

   A genuine Scope × OOS contradiction requires the SAME capability listed in BOTH as in scope AND as fully excluded WITHOUT exception clause, distinction, or disambiguation language. If you see any of "except", "exception", "salvo", "exceto", "unless", "other than", "with the exception of", or any in-scope item ID inside the OOS text, you are NOT looking at a contradiction.
</required_structural_patterns>

<required_architectural_patterns>
...
<final_calibration_check>
Before you commit a finding, ask yourself:

1. **If your evidence cites an OOS item, did you read it to its end?** An exclusion ending with "except for [in-scope item]" or equivalent ("exceto pelo X", "salvo o X", "unless", "other than", "with the exception of") makes the cited "contradiction" the contract's intentional disambiguation. Drop the finding. This check is FIRST because it is the dominant source of BLOCKER false positives in production.
2. Could this finding force the agent to rewrite content that follows a required pattern from sections A–C above? If yes, drop it.
3. Does the evidence cite two concrete anchors with quoted text? If not, severity cannot be `BLOCKER`.
4. Is the conflict mutually exclusive (both sides cannot hold), or merely a stylistic preference? If preference, the finding is at most `MINOR`, and probably not a finding at all.
5. Would a senior reviewer who knows the DAF/PSF style contract agree this is a defect — not a contractual requirement they recognize? If unsure, drop the finding.
</final_calibration_check>
</patterns_that_look_like_findings_but_are_not>
...
<example_structural_inconsistency_major>
```json
{
  "id": "F-002",
  "severity": "MAJOR",
...
<example_self_sufficiency_minor>
```json
{
  "id": "F-004",
  "severity": "MINOR",
