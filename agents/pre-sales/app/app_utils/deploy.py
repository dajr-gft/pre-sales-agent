...
def clean_python_caches(packages: list[str]) -> int:
    """Remove ``__pycache__`` directories and ``.pyc`` files from source packages.
    ...
    Bytecode caches generated locally add ~150 KB to the deploy payload
    and, on a project already close to the 8 MiB request limit, can push
    a single ``client.agent_engines.update`` call over the threshold.
    Bytecode is regenerated by the runtime on the server, so excluding
    the local copies is always safe.
    ...
    for pkg in packages:
        ...
        # recurse into a __pycache__ after we delete it.
...
def deploy_agent_engine_app(
    ...
    removed_caches = clean_python_caches(source_packages_list)
    if removed_caches:
        click.echo(
            f'\n🧹 Removed {removed_caches} bytecode cache '
            f'{"entry" if removed_caches == 1 else "entries"} from source '
