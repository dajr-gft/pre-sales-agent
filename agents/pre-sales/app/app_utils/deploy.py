...
def clean_deploy_caches(packages: list[str]) -> int:
    """Remove local-only cache directories and bytecode from source packages.

    The Agent Engine SDK tarballs ``source_packages`` with no exclusion
    filter (see ``_agent_engines_utils._create_base64_encoded_tarball``).
    Anything sitting next to source code at deploy time gets bundled into
    the payload. Three local-only artifact classes are stripped here:

    - ``__pycache__`` directories and ``.pyc`` files — Python bytecode,
      regenerated by the runtime on the server, ~150 KB per source tree.
    - ``.adk`` directories — created by ``adk web`` (local ADK Web UI
      session store). Can balloon to tens of MB across sessions and has
      pushed deploys over the 8 MiB ``agent_engines.update`` request
      limit in production. The runtime does not need it.

    Returns the number of cache entries removed (for logging).
    """
    removed = 0
    # Directory names that are local-only and must never ship in the deploy
    # tarball. Matched exactly against the directory name (not a path
    # substring), so an unrelated nested directory with the same name will
    # be removed too — which is the correct behavior since these names are
    # reserved by Python and ADK respectively.
    cache_dir_names = {'__pycache__', '.adk'}
    for pkg in packages:
        if not os.path.isdir(pkg):
            continue
        # topdown=True + prune dirs before descent so os.walk does not
        # recurse into a directory after we delete it.
...
def deploy_agent_engine_app(
    ...
    removed_caches = clean_deploy_caches(source_packages_list)
    if removed_caches:
        click.echo(
            f'\n🧹 Removed {removed_caches} local cache '
            f'{"entry" if removed_caches == 1 else "entries"} '
            '(__pycache__, .pyc, .adk) from source packages before bundling'
        )
