...
revision_agent could have applied — the failure mode observed when an
``auto_fixable`` finding rode alongside a single
``contradictions/assumptions_vs_risks`` (decision_required) into the
...
_CRITICAL_SKILLS = frozenset({'contradictions'})
...
def _evidence_discriminator(evidence: str) -> str | tuple[str, ...]:
    ...
    evidence — the canonical case, since contradictions /
...
def _fingerprint(f: Finding) -> str:
    ...
    - Otherwise → fall back to the legacy textual discriminator
      ...
      sorted fields. Reserved for categories whose only stable signal IS
      in the evidence (some ``semantic_quality`` cases, generic
      contradictions without structural identity).
    """
    key_pair = (f.skill, f.category)
    if key_pair in _FIELD_FINGERPRINT_CATEGORIES:
        key: tuple = (f.skill, f.category, tuple(sorted(f.fields or [])))
    else:
        key = (
            f.skill,
            f.category,
