# SOW Style Guide — cross-cutting writing rules (binding)

**Binding quality contract** for every SOW section produced by any workflow
skill. Only the rules that apply across all sections live here.
Section-specific rules (Executive Summary template, FR patterns, NFR WAF
pillars, OOS categories, assumption patterns, architecture rules,
workstream structure, roles rules) live in the matching workflow skill.

Language hygiene (conversation vs. final `.docx`, label translation,
`(inferred)` marker) is defined in `language-rules.md`. The rules below
apply equally to both surfaces — wording follows the language rules.

## General writing rules

- Clear, professional English. Active voice.
- Specific and quantifiable — no "up to", "various", "several". Use exact targets or narrowly defined ranges.
...
- Mark inferred content with the conversation-language `(inferred)` marker per `language-rules.md`.
- Never include hours, hourly rates, or rate cards anywhere in the document.
- Use scope-boundary language where applicable: "strictly limited to", "exclusively", "explicitly excluded".

---

## Self-sufficiency contract (cross-cutting — applies to FRs, NFRs, OOS, Assumptions)
...
## Paragraph breaks in long-form narrative
