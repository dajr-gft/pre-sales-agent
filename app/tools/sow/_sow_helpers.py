...
def download_gcs_uri(uri: str) -> bytes | None:
    """Download raw bytes from a gs:// URI.

    Args:
        uri: A GCS URI like 'gs://bucket-name/path/to/object'.
    ...
    try:
        ...
        if not uri.startswith('gs://'):
            logger.warning('download_gcs_uri: not a gs:// URI | uri=%s', uri)
            return None
        ...
        if not bucket_name or not blob_name:
            logger.warning(
                'download_gcs_uri: could not parse URI | uri=%s', uri
            )
            return None
        ...
        logger.info(
            'download_gcs_uri: downloaded | bucket=%s | blob=%s | size=%d',
            bucket_name,
            blob_name,
            len(data),
        )
        ...
        logger.error(
            'download_gcs_uri: failed | uri=%s | error=%s | type=%s',
            uri,
            str(err),
            type(err).__name__,
        )
